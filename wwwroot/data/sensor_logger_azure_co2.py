...
def parse_sensor_data(data: str) -> tuple[str | None, float, float, float, float | None] | None:
    ...
    co2 = float(match.group(2)) if match.group(2) else None
    temp_c = float(match.group(3))
    temp_f = float(match.group(4))
    humidity = float(match.group(5))

    return device_name, temp_c, temp_f, humidity, co2
        ...
        while True:
            for sensor_ip in SENSORS:
                try:
                    ...
                    device_name, temp_c, temp_f, humidity, co2 = parsed
